...
// Define Global Augmentation Templates
// These parameters should align with what Augmentation.js expects in initialParameters
const globalAugmentationTemplates = [
    {
        ...
        settings: {
            ...
            parameters: { // Only specify parameters that deviate from Augmentation.js's initialParameters or are specific to these techniques
    ...
    },
    {
        ...
        settings: {
            ...
            parameters: {
                rotation_angle: 45, // Example: rotate by 45 degrees
                scaling_factor: 1.1, // Example: scale by 10%
                // flip_horizontal has no parameters
            }
        }
    },
    {
        key: "global_noise_occlusion", // Renamed for clarity
        name: "Global Preset: Noise & Occlusion",
        settings: {
            techniques: ["gaussian_noise", "salt_pepper_noise", "cutout"], // Removed motion_blur to keep it simpler
            parameters: {
                gaussian_variance: 0.02, // Slightly more noise
                sap_amount: 0.006,      // Slightly more salt & pepper
                cutout_size: 40,        // Moderate cutout
            }
        }
    }
];
...
const Uploads = () => {
  ...
  const [datasets, setDatasets] = useState([]);
  const [datasetName, setDatasetName] = useState('');
  const [selectedTemplateKey, setSelectedTemplateKey] = useState(''); // For dataset-specific run templates
  const [selectedGlobalTemplateKey, setSelectedGlobalTemplateKey] = useState(''); // For global templates
  const navigate = useNavigate();
  ...
  const handleDatasetTemplateChange = (e) => {
    ...
    if (e.target.value) {
        setSelectedGlobalTemplateKey(''); // Reset global template if dataset template is chosen
    }
  };
  ...
  const handleGlobalTemplateChange = (e) => {
    ...
    if (e.target.value) {
        setSelectedTemplateKey(''); // Reset dataset template if global template is chosen
    }
  };
  ...
  const commonSelectStyle = { marginRight: '10px', padding: '8px', borderRadius: '4px', border: '1px solid #ccc', minWidth: '200px', marginBottom:'5px' };
  const templateSectionStyle = { marginTop: '20px', marginBottom: '20px', padding: '15px', border: '1px solid #eee', borderRadius: '4px', background: '#f9f9f9' };
    ...
    <div style={{ fontFamily: 'Arial, sans-serif', padding: '20px' }}>
      ...
      <div style={{display: 'flex', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap'}}>
        <input
            ...
            value={datasetName}
            onChange={(e) => setDatasetName(e.target.value)}
            style={{ ...commonSelectStyle, marginRight: '10px' }}
        />
        <input 
            type="file" 
            multiple 
      ...
      <div style={templateSectionStyle}>
        <h3>Choose Augmentation Settings Template (Optional)</h3>
        <p style={{fontSize: '0.9em', color: '#555', marginTop: 0}}>
            Select a template to pre-fill settings on the augmentation page. Choosing one will deselect the other.
        </p>
        <div style={{display: 'flex', flexWrap: 'wrap', gap: '15px'}}>
            <div>
                <label htmlFor="globalTemplateSelect" style={{display: 'block', marginBottom: '3px', fontSize: '0.9em'}}>Global Presets:</label>
                <select
            ...
            <div>
                <label htmlFor="datasetTemplateSelect" style={{display: 'block', marginBottom: '3px', fontSize: '0.9em'}}>Copy from Past Dataset Run:</label>
                <select
        ...
        <tbody>
          {datasets.map((ds) => (
            <tr key={ds.name}>
              ...
              <td style={{padding: '8px'}}>
                {ds.files && ds.files.slice(0, 3).map((file, j) => (
                  <div key={j} style={{ margin: '2px 0', wordBreak: 'break-all', fontSize: '0.9em' }}>
                    {file.match(/\.(jpg|jpeg|png)$/i) ? (
                      <img
                        src={`http://localhost:5001/uploads/${ds.name}/${encodeURIComponent(file)}`}
                        alt={file.length > 20 ? file.substring(0,17) + '...' : file}
                        title={file}
                        width="30"
                        style={{ verticalAlign: 'middle', marginRight: '5px', border:'1px solid #eee' }}
                        onError={(e) => {e.target.style.display='none'; e.target.nextSibling.textContent = file + ' (err)'; }}
                      />
                    ) : null}
                     {file.length > 20 ? file.substring(0,17) + '...' : file}
                  </div>
                ))}
                {ds.files && ds.files.length > 3 && <div style={{marginTop: '5px', color: '#777', fontSize: '0.9em'}}>...{ds.files.length - 3} more.</div>}
              </td>
              <td style={{padding: '8px'}}>
                {ds.augmentation_runs && ds.augmentation_runs.length > 0 ? (
                  <ul style={{ paddingLeft: '15px', margin: 0, listStyleType: 'disc', fontSize: '0.9em' }}>
                    {ds.augmentation_runs.slice().reverse().map((run, index) => (
                      <li key={index} style={{ marginBottom: '8px', borderBottom: index < ds.augmentation_runs.length -1 ? '1px dashed #eee' : 'none', paddingBottom: '5px' }}>
                        <strong>{run.run_id}</strong> ({run.timestamp}):&nbsp;
                        <a 
                            href={`http://localhost:5001/augmented/${ds.name}/${encodeURIComponent(run.augmented_zip)}`} 
